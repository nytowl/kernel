...
#define SD_OFFSET_SECS_LOGO2			45056
#define SD_OFFSET_SECS_WAVEFORM		14336

#define SD_OFFSET_SECS_SN		1

#define DEFAULT_LOAD_KERNEL_SZ	20000
#define DEFAULT_LOAD_RD_SZ	20000
...
#define GET_SD_NUM(_i)	({\
	if(MMC_NUM_NUKOWN==gi_mmc_##_i##sd_num) {\
		...
		else if(11==_get_pcba_id()||35==_get_pcba_id()||41==_get_pcba_id()) {\
			/* E606CX/E606FXB */ \
			gi_mmc_esd_num = 0;\
		}\
		else {\
...
static GPIO_OUT gt_ntx_gpio_col0= {
	MX50_PIN_KEY_COL0,  // pin name .
...
static void _load_ntxbins_and_append_boot_args(char *O_cBufA,unsigned long I_ulBufSize)
{
	...
	char *szLoadNameA[] = {(char *)gszKParamName_HwCfg,(char *)gszKParamName_waveform,(char *)gszKParamName_logo,(char *)gszKParamName_logo};
	// ] 
	
	int iLastLoadSecOffset;
	unsigned long dwLastLoadSecs;
	int i;
...
static int _detect_bootmode(void)
{
	...
	if(NTXHWCFG_TST_FLAG(gptNtxHwCfg->m_val.bPCB_Flags,0)) {
		//NO keymatrix .
		//  KB_COL0 output low ,to fix E606E0A1 PCB bug ...
		gpio_out_init(&gt_ntx_gpio_col0);

		/*
		mxc_request_iomux(MX50_PIN_KEY_COL0, IOMUX_CONFIG_ALT1);
		// set as output direction .
	...
	case 18://E50600.
		...
		break;
	case 27:
		iRet = ntxup_wait_touch_recovery();
		if(1 == iRet) {
			printf("\n**********************************\n\n");
		...
		else if(ntxup_is_ext_card_inserted() && 2 == iRet) {
			printf("\n**************************\n\n");
			printf("\n**  1. Boot from ESD    **\n\n");
			printf("\n**************************\n\n");		
			iRet = NTX_BOOTMODE_ESD_UPG;
...
#if 0
	{
		...
		else if(1==(iUPG_Key=ntxup_wait_key_esdupg())) {
			printf("\n**********************************\n\n");
			printf("\n**  Boot from SD quick recovery **\n\n");
			printf("\n**********************************\n\n");		
			iRet = NTX_BOOTMODE_RECOVERY;
...
static int do_load_ntxbins(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	...
	gi_mmc_num_kernel = GET_ISD_NUM() ;
	sprintf(cCmdA," boot_port=%d",gi_mmc_num_kernel);
	strcat(gszBootArgs,cCmdA);
